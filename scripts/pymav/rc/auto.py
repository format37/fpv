...
def get_rc_switch_state(vehicle, channel=6, threshold=1500):
    rc_value = vehicle.channels[str(channel)]
    return rc_value > threshold if rc_value else False
...
def main():
    ...
    iterator = 0
    while True:
        print(f"{iterator}: {vehicle.channels[str(7)]}")
        time.sleep(0.1)
        iterator += 1
